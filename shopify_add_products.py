...
def upload_new_products(new_products):
    ...
    for i, product in enumerate(new_products):
        try:
            ...
            dir_list = os.listdir(os.path.join('pictures', product["picture_folder"]))
            print(dir_list)
            images = []
            for i in dir_list:
                image = shopify.Image()
                with open(f'pictures/{product["picture_folder"]}/{i}',"rb") as f:
                    encoded = f.read()
            ...
            print("[Error] Failed add Project")
            mk.append(product)
    print("------------------ COMPLETED UPLOADING PRODUCTS ------------------")
    print("------------------ RSULT OF UPLOADING PRODUCTS ------------------")
    if mk == []:
        print("[Info] Successfully uploading all Project")
    else:
        print(f'[Error] Failed in uploading projects: {len(mk)}')
        for i, mk_item in enumerate(mk):
            print(f'{i + 1}: {mk_item["title"]}')
    return mk
...
if __name__ == "__main__":
    with open(NEW_CHECKED_PRODUCT) as f:
        ...
        f.close()
    failed_upload = upload_new_products(new_upload_product)

    with open(NEW_CHECKED_PRODUCT, "w") as f:
        json.dump(failed_upload, f, indent=2)
        f.close()
